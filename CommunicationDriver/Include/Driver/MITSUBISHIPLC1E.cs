...
namespace CommunicationDriver.Include.Driver
{
	public class CMITSUBISHIPLC1E : IMitsubishiPlc
	{
        ...
        public int ReadBitsPLC(ref bool[] bits, DEVICETYPE eHeader, int bitStartAddress, int iNUMOfDevice = 128)
        {
            if (iNUMOfDevice <= 0) return 0;

            // 1) 將 bit 起點換成 word 起點與位移
            int wordStart = bitStartAddress / 16;      // 起始 word
            int bitOffset = bitStartAddress % 16;      // 起始 word 的位移
            int totalBits = bitOffset + iNUMOfDevice;
            int wordCount = (totalBits + 15) / 16;     // 需要讀的 word 數（ceil）

            // 2) 讀取這些 word
            short[] inputBuffer = new short[wordCount];
            int rc = ReadBatchPLC(ref inputBuffer, eHeader, wordStart * 16, wordCount);
            if (rc < 0) return rc;                     // 錯誤直接回傳
                                                       // 3) word→bits（展開到暫存 newBits）
            bool[] allBits = new bool[wordCount * 16];
            for (int w = 0; w < wordCount; w++)
            {
                ushort val = unchecked((ushort)inputBuffer[w]);
                for (int b = 0; b < 16; b++)
                    allBits[w * 16 + b] = ((val >> b) & 1) != 0; // LSB→MSB
            }

            bool[] newBits = new bool[iNUMOfDevice];
            Array.Copy(allBits, bitOffset, newBits, 0, iNUMOfDevice);

            // 4) 到這裡代表成功，才覆寫呼叫者的 bits
            bits = newBits;
            return bits.Length;
        }
